...
class QdhrRegistrationTests(unittest.TestCase):
    def test_source_catalog_contains_qdhr(self) -> None:
        source_info = get_source_info("qdhr")

        self.assertTrue(source_info["enabled"])
        self.assertEqual(source_info["platform_code"], "qdhr")
        self.assertEqual(source_info["strategy"], "requests_html")

    def test_source_scripts_contains_qdhr_script(self) -> None:
        script_path = SOURCE_SCRIPTS.get("qdhr")
